...
// Mỗi tab có một màu chip nhạt riêng cho icon; tab đang chọn dùng pill xanh brand đậm.
const TABS = [
  { id: 'home', ic: '🏠', label: 'Trang chủ', tint: '#DCF2E4' },
  { id: 'journey', ic: '📅', label: 'Hành trình', tint: '#DCE9FB' },
  { id: 'auction', ic: '🏷️', label: 'Green Auction', tint: '#FBEBD1' },
  { id: 'wall', ic: '🌍', label: 'Green Wall', tint: '#D8EEF7' },
  { id: 'meter', ic: '📊', label: 'Green Meter', tint: '#DCEFD9' },
  { id: 'profile', ic: '🎖️', label: 'Hồ sơ', tint: '#FBE1D2' },
];

const ROLE_LABEL = { student: 'Học sinh', teacher: 'Giáo viên', admin: 'Quản trị viên' };

const CARD = 'rounded-[20px] bg-white shadow-card ring-1 ring-black/[.03]';
    ...
    return (
      <div className="relative flex min-h-screen flex-col items-center justify-center gap-3 px-6 text-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          ...
          <button onClick={handleLogout} className="mt-2 rounded-xl bg-brand px-5 py-2.5 text-sm font-bold text-white hover:brightness-105">Đăng xuất</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <main className="relative z-10 mx-auto w-full max-w-[880px] px-4 pb-28 pt-6 sm:px-6 md:pb-14 md:pt-8">
        {/* Thanh tab dạng pill trên nền trắng (desktop) */}
        <nav aria-label="Điều hướng chính"
          className="mb-7 hidden items-center gap-1.5 rounded-[18px] bg-white p-1.5 shadow-card ring-1 ring-black/[.03] md:flex">
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-2 rounded-2xl px-3 py-2.5 text-center text-[13.5px] font-bold leading-snug transition-colors ${active ? 'bg-brand text-white shadow-soft' : 'text-ink-700 hover:bg-canvas'}`}
              >
                <span aria-hidden="true" className="flex h-6 w-6 items-center justify-center rounded-lg text-[14px]"
                  style={active ? { backgroundColor: 'rgba(255,255,255,.22)' } : { backgroundColor: t.tint }}>{t.ic}</span>
                {t.label}
              </button>
            );
          })}
      ...
      <nav aria-label="Điều hướng chính"
        className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-6 gap-1 border-t border-line bg-white/95 px-1.5 pt-2 shadow-navbar backdrop-blur md:hidden"
        style={{ paddingBottom: 'calc(8px + env(safe-area-inset-bottom,0px))' }}>
        {TABS.map((t) => {
          const active = tab === t.id;
          return (
            <button key={t.id} type="button" onClick={() => setTab(t.id)} aria-current={active ? 'page' : undefined}
              className={`flex flex-col items-center gap-0.5 rounded-xl px-0.5 py-1.5 text-center text-[10px] font-semibold leading-tight ${active ? 'text-brand-700' : 'text-ink-600'}`}>
              <span aria-hidden="true" className={`flex h-7 w-7 items-center justify-center rounded-lg text-[16px] ${active ? 'ring-2 ring-brand/70' : ''}`}
                style={{ backgroundColor: t.tint }}>{t.ic}</span>
              {t.label}
            </button>
          );
        })}
      ...
      {toast && (
        <div role="status" className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 animate-rise whitespace-nowrap rounded-full bg-brand-700 px-5 py-3 text-sm font-semibold text-white shadow-pop md:bottom-6">
          ✅ {toast}
        </div>
      )}
    </div>
  ...
  return (
    <div className="md:grid md:grid-cols-[1.32fr_1fr] md:items-start md:gap-6">
      <div className="relative">
        {student.streak > 0 && (
          <div className="absolute -top-3.5 right-5 z-10 flex items-center gap-1.5 rounded-full bg-honey px-3.5 py-1.5 text-[12px] font-bold text-white shadow-soft">
            <span aria-hidden="true">🔥</span> Streak {student.streak} ngày
          </div>
        )}
        <section aria-label="Green Passport"
          className="relative overflow-hidden rounded-[24px] p-6 text-white shadow-hero"
        ...
        <button type="button" onClick={onRecord}
          className={`mt-4 flex w-full items-center gap-4 rounded-2xl px-4 py-3.5 text-left transition-[filter,transform] hover:brightness-105 active:translate-y-px ${doneToday ? 'border border-line bg-white text-ink shadow-soft' : 'bg-gradient-to-r from-brand to-brand-700 text-white shadow-soft'}`}>
          {doneToday ? (
        ...
        {choice ? (
          <>
            <div className="mt-3 rounded-xl bg-canvas px-3.5 py-3">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-line"><div className="h-full rounded-full bg-gradient-to-r from-mint-500 to-brand" style={{ width: `${pct}%` }} /></div>
            </div>
          ...
          <div className="mt-4 flex gap-2.5">
            <button type="button" onClick={() => onJoin(ch.id, 'in')} className="flex-1 rounded-xl bg-brand py-2.5 text-[13px] font-bold text-white hover:brightness-105">🟢 I'M IN</button>
            <button type="button" onClick={() => onJoin(ch.id, 'skip')} className="flex-1 rounded-xl border border-line bg-white py-2.5 text-[13px] font-bold text-ink-700 hover:bg-canvas">🔴 NOT YET</button>
          </div>
        )}
      </div>
    </div>
...
function StatCard({ icon, tint, num, lbl }) {
  return (
    <div className={`${CARD} p-4`}>
      ...
      <div className="mt-3 text-[26px] font-extrabold leading-none text-brand-700">{num}</div>
  ...
  return (
    <div>
      ...
      <div className={`${CARD} p-6`}>
        <div className="text-[17px] font-bold">Lớp {CLASS_METER.classroom}</div>
        ...
        <div className="mt-5 h-5 overflow-hidden rounded-full bg-line">
          <div className="flex h-full items-center justify-end rounded-full pr-2.5 text-[11px] font-bold text-white"
            style={{ width: `${pct}%`, background: 'linear-gradient(90deg,#64BF9C,#257167)' }}>{pct}%</div>
        </div>
        <div className="mt-5 text-center text-[14px] font-bold text-brand-700">“Tôi hành động → Lớp tiến lên.”</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className={`${CARD} p-5`}>
        ...
        <button type="button" onClick={onLogout} className="mt-4 w-full rounded-xl border border-line bg-white py-2.5 text-sm font-bold text-red-600 hover:bg-red-50">Đăng xuất</button>
      </div>
    </div>
  );
}
    ...
    <div role="dialog" aria-modal="true" aria-label="Ghi nhận hành động hôm nay"
      ...
      <div className="max-h-[90vh] w-full max-w-md overflow-auto rounded-t-[24px] bg-white p-6 shadow-pop sm:rounded-[24px]">
        ...
        <div className="mb-4">
          ...
          {ch.actions.map((a, i) => (
            <label key={i} className={`mb-2 flex cursor-pointer items-center gap-2.5 rounded-xl border px-3.5 py-3 text-[13px] font-semibold ${recordChoice === i ? 'border-brand bg-brand-soft' : 'border-line hover:bg-canvas'}`}>
        ...
        <div className="mb-5">
          ...
          <textarea id="evidence-note" rows={2} value={note} onChange={(e) => setNote(e.target.value)}
            ...
            className="w-full rounded-xl border border-line px-3.5 py-2.5 text-[13.5px] focus:border-brand focus:outline-none" />
        </div>
        <button type="button" onClick={onSubmit} disabled={recordChoice === null || submitting}
          className="w-full rounded-xl bg-gradient-to-r from-brand to-brand-700 py-3 text-[14px] font-bold text-white hover:brightness-105 disabled:opacity-50">
