...
export default function StudentPage() {
  ...
  const loadAll = useCallback(async (userId) => {
    ...
    let { data: studentRow } = await supabase.from('students').select('*').eq('id', userId).single();

    // Đăng nhập lần đầu (vd qua Google) chưa có hồ sơ trong users/students — tự tạo ngay,
    // lấy tên hiển thị từ thông tin tài khoản Google (user_metadata) nếu có.
    if (!userRow) {
      const { data: authData } = await supabase.auth.getUser();
      const meta = authData?.user?.user_metadata || {};
      const fullName = meta.full_name || meta.name || authData?.user?.email || 'Học sinh mới';
    ...
    if (!studentRow) {
      const { data: newStudentRow } = await supabase
        .from('students')
        .insert({ id: userId, streak: 0, evidence_count: 0, current_day: 1 })
        .select()
        .single();
